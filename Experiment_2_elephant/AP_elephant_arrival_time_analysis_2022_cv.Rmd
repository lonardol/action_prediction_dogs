...
sample_data <- read_parquet("sample_report")%>%
...
arrival_time_plot_try  <- ggplot(data = targetIA_gaze_data.agg, aes(x = condition, y= mean_arrival_time)) +
...
```
```{r}
ggsave(arrival_time_plot_first_trial2, filename="graphics/arrival_time_barplot_first_trial.png",width = 8, height = 8, scale = 0.6)
```
...
summary(model1_beta)
overdisp.test(model1_beta) #not overdispersed 0.822
...
ggsave(filename="graphics/looking_times_standardised_cond_IA.png",
  plot = last_plot(),
  width=8,
  height=5,
  dpi=1200)

p.std.main.eff.con<-ggplot(data=dyn_ia_data,
...
library(cowplot)
plot_grid(p.d2, p.hw2, p.ho2, nrow=1)
...
```
#post-hoc pairwise comparisons (in the manuscript I reported the results of the re-leveled model)

```{r}
library(multcomp)
summary(glht(mm_abs_looking_time, linfct = mcp(condition= c("human_within - dog = 0", 
                                      "human_outside - dog = 0",
                                      "human_within - human_outside = 0"))))
```
...
CV: we don't need this model I think
...
dyn_ia_data$std_looking_time_norm=dyn_ia_data$standardised_dwell_time/8000
min(dyn_ia_data$std_looking_time_norm)#min =  0
max(dyn_ia_data$std_looking_time_norm)#max smaller than 1

#remove 0s (and 1s)
dyn_ia_data$resp <- (dyn_ia_data$std_looking_time_norm*(length(dyn_ia_data$std_looking_time_norm) - 1) + 0.5)/length(dyn_ia_data$std_looking_time_norm) 
...
#coefficients
summary(betam_prop_std_looking_time)
#p-values
drop1(betam_prop_std_looking_time, test="Chisq") #does not work
#overdispersion
overdisp.test(betam_prop_std_looking_time) #0.86
```
model stability
```{r}
source("./functions/glmmTMB_stability.r")
...
#z-transformation
model_data2$z.trial=scale(model_data2$TRIAL_INDEX)
model_data2$z.order=scale(model_data2$order)

summary(model_data2)

model_data2<-model_data2%>%filter(!is.na(min_arrival_time))
#dummy coding and centering the non-reference levels of condition
model_data2$condition_dummy1.c=scale(as.numeric(model_data2$condition==levels(as.factor(dyn_ia_data$condition))[2]), center=TRUE, scale= FALSE)
model_data2$condition_dummy2.c=scale(as.numeric(model_data2$condition==levels(as.factor(dyn_ia_data$condition))[3]), center=TRUE, scale= FALSE)
```
...
library(glmmTMB)
contr<-glmmTMBControl(optCtrl=list(iter.max=100000000, eval.max=100000000))
betam_prop_time_gaze_target=glmmTMB(min_arrival_time_norm~condition + z.trial + z.order + (1+condition_dummy1.c+condition_dummy2.c+z.trial||subject), family=beta_family, data=model_data2,
      control=contr)
#df, chisq, p-val
