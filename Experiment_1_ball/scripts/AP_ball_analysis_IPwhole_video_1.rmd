...
sample_data <- read_parquet("data/ball_sample_report_arrival_looking")%>%
...
#  geom_half_violin(data = walk.away.data %>% filter(condition == "dog""), aes(x = condition2, y = mean_prop_away_time), position = position_nudge(x = 2.8), side = "r", width = 0.5, fill = "dodgerblue", alpha = .3) +
 ...
 geom_half_boxplot(
     ...
     side = "r",outlier.shape = NA, center = TRUE, errorbar.draw = TRUE, varwidth = FALSE, 
     fill = 'dodgerblue', alpha = .5) +
  geom_errorbar(data = ci.data %>% filter(condition == "dog"), aes(x = 1.395, y = fitted, ymin = lower.cl, ymax = upper.cl), color = "black", width = 0.1, size = 0.7)+
   geom_point(data = ci.data %>% filter(condition == "dog"), aes(x = 1.395, y = fitted),color= "black", cex=2)+
   ...
   geom_half_boxplot(
     ...
     side = "r",outlier.shape = NA, center = TRUE, errorbar.draw = TRUE, varwidth = FALSE, 
     fill = 'darkorange', alpha = .5) +
   geom_errorbar(data = ci.data %>% filter(condition == "human_within"), aes(x = 3.39, y = fitted, ymin = lower.cl, ymax = upper.cl), color = "black", width = 0.1, size = 0.7)+
   geom_point(data = ci.data %>% filter(condition == "human_within"), aes(x = 3.39, y = fitted),color= "black", cex=2)+
     ...
     side = "r",outlier.shape = NA, center = TRUE, errorbar.draw = TRUE, varwidth = FALSE, 
     fill = 'darkgreen', alpha = .5) +
  geom_errorbar(data = ci.data %>% filter(condition == "human_outside"), aes(x = 2.334, y = fitted, ymin = lower.cl, ymax = upper.cl), color = "black", width = 0.1, size = 0.7) +
  geom_hline(yintercept = mean(model_data$agent_arrival_time, na.rm=TRUE)/8000, lty=2, col="red")+
     geom_point(data = ci.data %>% filter(condition == "human_outside"), aes(x = 2.334, y = fitted),color= "black", cex=2)+
 ...
 # ylim(0, 1) +
   geom_segment(aes(x = 1.4, y = 0.87, xend = 3.4, yend = 0.87))+
  geom_text(x=2.35, y=0.9, label="*", size=6)+ #t.test pairwise comparison between conditions
  theme_classic()
...
#  geom_half_violin(data = walk.away.data %>% filter(condition == "dog""), aes(x = condition2, y = mean_prop_away_time), position = position_nudge(x = 2.8), side = "r", width = 0.5, fill = "dodgerblue", alpha = .3) +
 ...
 geom_half_boxplot(
   ...
   geom_half_boxplot(
     ...
     fill = 'darkgreen', alpha = .5) +
  geom_hline(yintercept = 0, lty=2)+
  geom_signif(comparisons=list(c("dog", "human_outside")), annotations="*",
...
arrival_time_plot_first_trial2  <- ggplot(data = targetIA_gaze_data_first_trial_mean_names, aes(x = condition, y= mean, fill=condition)) +
  ...
  guides(fill = FALSE)+
  geom_text(x=1, y=1700, label="*", size=6, col="red")+ #t-test against 0
  geom_text(x=2, y=1700, label="*", size=6, col="red")+ #t-test against 0
  geom_signif(comparisons=list(c("Dog","Human outside")), annotations="**",
              y_position = 1800, tip_length = 0) +
  geom_signif(comparisons=list(c("Dog","Human within")), annotations="*",
              y_position = 2200, tip_length = 0)
  
arrival_time_plot_first_trial2
```
#save plot
```{r}
ggsave(plot=arrival_time_plot_first_trial2, filename="graphics/arrival_times_barplot_1st_trial_ball.png", dpi=1200, scale=0.6, width=6, height=8)
```
...
table(model_data$condition,is.na(model_data$min_arrival_time))
NA_trials<-table(model_data$condition,model_data$arrival_time_standardised=="NA")
```
#fit a binomial model to check that the probability that dogs do not look at the target in a trial does not differ significantly across conditions
```{r}
...
round(summary(bn.m.notarget)$coefficients,2)
res.bn.m.notarget<-round(drop1(bn.m.notarget, test="Chisq"),3)
res.bn.m.notarget
```

```{r}
#exclude trials in which dogs never looked at the target for gaze arrival times analysis
...
overdisp.test(model1_beta) #not overdispersed 0.784
...
```
#confidence intervals for plot (requires manual dummy coding and data as data frame)
```{r}
source("./functions/boot_glmmTMB.r")
...
```
## beta model (because the assumptions of LMM1 were not met) Is this a REPETITION OF WHAT WAS ALREADY ABOVE? L. 465?
...
p.d<-ggplot(data=dyn_ia_data%>%filter(condition=="dog"), aes(x=IA, y=standardised_dwell_time))+
 ...
 geom_signif(comparisons=list(c("face","body")), annotations="*",
              y_position = 0.40, tip_length = 0.02) 
p.d
...
ggsave(filename="graphics/looking_times_standardised_cond_IA_ball_CIs_signif.png",
  plot = last_plot(),
  width=8,
  height=5,
  dpi=1200)
...
#for plot
boot.mm.abs.lt_for_plot=boot.glmm.pred(model.res=mm_abs_looking_time2, use="condition", nboots=1000, para=T)
boot.mm.abs.lt_for_plot$ci.predicted
```

#### Standardized looking times: LMM 
...
drop1(betam_std_looking_time2, test="Chisq") # model convergence warning (due to covariate z.order)
...
lsmeans(betam_std_looking_time2, pairwise~condition)
```
model stability
```{r}
source("./functions/glmmTMB_stability.r")
...
betam_std_looking_time2_ci_plot=glmmTMB(resp~condition*IA + z.trial + z.order + 
...
betam_std_looking_time2_ci_plot_pred=boot.glmmTMB(betam_std_looking_time2_ci_plot,data=dyn_ia_data, nboots=1000, para=T, resol=1000, level=0.95, n.cores = "all-1", 
	use="condition", contr=glmmTMBControl(optCtrl=list(iter.max=100000000, eval.max=100000000)))

ci.data_std_lt<-betam_std_looking_time2_ci_plot_pred$ci.fitted$ci.fitted.cond
ci.data_std_lt
```
